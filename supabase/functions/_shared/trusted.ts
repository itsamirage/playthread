...
export async function assertNoBannedSignals(
  adminClient: SupabaseClient,
  request: Request,
  userId: string,
) {
  const ipHash = await getRequestIpHash(request);
  const signalHashes: Array<{ signal_type: "network" | "device"; signal_hash: string }> = [];

  if (ipHash) {
    signalHashes.push({ signal_type: "network", signal_hash: ipHash });
  }
  ...
  for (const row of tokenRows ?? []) {
    if (row.expo_push_token) {
      signalHashes.push({
        signal_type: "device",
        signal_hash: await hashValue(`device:${row.expo_push_token}`),
      });
    }
  }

  for (const signal of signalHashes) {
    const { data, error } = await adminClient
      .from("moderation_ban_signals")
      .select("id")
      .eq("signal_type", signal.signal_type)
      .eq("signal_hash", signal.signal_hash)
      .eq("is_active", true)
      .limit(1);
    ...
    if ((data ?? []).length > 0) {
      throw new Error("This network or device is banned. Posting and reactions are disabled.");
    }
  }
}
...
function matchesModerationPatterns(patterns: RegExp[], rawText: string, normalizedText: string) {
...
export async function createModerationFlag(
  adminClient: SupabaseClient,
  {
    ...
    userId: string;
    category: "hate" | "abuse" | "nudity" | "spam" | "integrity";
    labels: string[];
    reason: string;
