  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: commentsError } = await supabase
        .from("post_comments")
        .select(
          "id, post_id, user_id, body, image_url, moderation_state, moderation_labels, created_at, updated_at, profiles(username, display_name)"
...
export async function createPostComment({ userId, postId, body, imageAsset = null }) {
...
export async function updatePostComment({ commentId, body }) {
  try {
    const data = await invokeEdgeFunction("trusted-comment", {
      action: "update",
      commentId,
