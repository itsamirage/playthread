...
function parsePostImageUrls(imageUrlsValue, fallbackImageUrl = null) {
...
function normalizePost(post) {
  ...
  const developerGameIds = Array.isArray(post.profiles?.developer_game_ids)
    ? post.profiles.developer_game_ids.map((value) => Number(value)).filter((value) => !Number.isNaN(value))
    : [];
    ...
    supabase
      ...
      .select(
        "id, user_id, igdb_game_id, game_title, game_cover_url, type, title, body, reaction_mode, rating, comments_count, moderation_state, moderation_labels, created_at, updated_at, spoiler, spoiler_tag, profiles(username, display_name)"
          .replace("profiles(username, display_name)", "image_url, image_urls, pinned_until, video_provider, video_upload_id, video_upload_token, video_asset_id, video_playback_id, video_status, video_thumbnail_url, video_duration_seconds, profiles(username, display_name, selected_title_key, selected_name_color, developer_game_ids)")
      )
  ...
  try {
    ...
    if (normalizedImageAssets.length > 0) {
      const uploads = await Promise.all(
        normalizedImageAssets.map((asset) =>
          uploadPostImage({
            userId,
            asset,
          }),
