...
import { normalizeNumericIdArray, parsePostImageUrls } from "./postNormalization.js";
import { supabase } from "./supabase";

function useRealtimeReload({ enabled, tables, reload }) {
  const channelIdRef = useRef(`realtime:${Math.random().toString(36).slice(2)}`);
...
export const POST_TYPE_OPTIONS = ["discussion", "review", "screenshot", "guide", "tip", "clip"];
...
export const POST_TOP_PERIOD_OPTIONS = {
  week: "This week",
  month: "This month",
  all: "All time",
};
...
function getTopPeriodStart(period) {
  const now = Date.now();
...
function normalizePost(post) {
  ...
  const pinnedUntil = post.pinned_until ?? null;
  const developerGameIds = normalizeNumericIdArray(post.profiles?.developer_game_ids);

  const imageUrls = parsePostImageUrls(post.image_urls, post.image_url ?? null);
  const imageCaptions = Array.isArray(post.image_captions)
    ? post.image_captions.map((value) => String(value ?? "").trim())
    : [];
  ...
  return {
    ...
    externalVideoProvider: post.external_video_provider ?? null,
    externalVideoId: post.external_video_id ?? null,
    externalVideoUrl: post.external_video_url ?? null,
    externalVideoTitle: post.external_video_title ?? null,
    externalVideoThumbnailUrl: post.external_video_thumbnail_url ?? null,
    spoiler: Boolean(post.spoiler),
    spoilerTag: post.spoiler_tag ?? null,
    isNsfw: Boolean(post.is_nsfw),
    rating:
      typeof post.rating === "number"
        ? Number(post.rating.toFixed(1))
        : post.rating
          ? Number(Number(post.rating).toFixed(1))
          : null,
    author:
    ...
    age: formatRelativeTime(createdAt),
    createdAt,
    updatedAt,
    pinnedUntil,
    isPinned: Boolean(pinnedUntil && new Date(pinnedUntil).getTime() > Date.now()),
    isDeveloperPost: developerGameIds.includes(Number(post.igdb_game_id)),
    isEdited: Boolean(updatedAt && createdAt && new Date(updatedAt).getTime() > new Date(createdAt).getTime() + 1000),
  };
}

export const normalizePostForExternal = normalizePost;
...
function normalizeComment(comment, userId) {
  ...
  return {
    ...
    linkUrl: comment.link_url ?? null,
    linkLabel: comment.link_label ?? null,
    age: formatRelativeTime(createdAt),
    createdAt,
    updatedAt,
    isEdited: Boolean(updatedAt && createdAt && new Date(updatedAt).getTime() > new Date(createdAt).getTime() + 1000),
...
export const normalizeCommentForExternal = normalizeComment;
...
export async function loadExternalPostsWithReactions(posts, userId) {
  return loadReactionState(posts, userId);
}
...
export function filterPostsByTypes(posts, selectedTypes) {
  if (!Array.isArray(selectedTypes) || selectedTypes.length === 0) {
    return posts;
  }

  const allowedTypes = new Set(selectedTypes);
  return posts.filter((post) => allowedTypes.has(post.type));
}

export function sortPostsForDisplay(posts, { sort = "new", topPeriod = "week" } = {}) {
  ...
  if (sort === "new") {
    return [...posts].sort(
      (left, right) =>
        sortPinnedFirst(left, right) ||
        new Date(right.createdAt).getTime() - new Date(left.createdAt).getTime(),
    );
  }

  if (sort === "top") {
    const period = topPeriod === "month" ? "month" : topPeriod === "all" ? "all" : "week";
    const minTimestamp = getTopPeriodStart(period);
    const scopedPosts = minTimestamp
      ? posts.filter((post) => new Date(post.createdAt).getTime() >= minTimestamp)
      : posts;

    return [...scopedPosts].sort(
      (left, right) =>
        sortPinnedFirst(left, right) ||
        scorePost(right, { period }) - scorePost(left, { period }) ||
        new Date(right.createdAt).getTime() - new Date(left.createdAt).getTime(),
    );
  }

  return [...posts].sort((left, right) => {
    const pinnedSort = sortPinnedFirst(left, right);
    if (pinnedSort !== 0) {
      return pinnedSort;
    }
    const rightScore = scorePost(right, { period: "day" });
    const leftScore = scorePost(left, { period: "day" });
...
export function useFeedPosts(followedGameIds) {
  ...
  const buildQuery = useCallback((offset) =>
    supabase
      .from("posts")
      .select(
        "id, user_id, igdb_game_id, game_title, game_cover_url, type, title, body, reaction_mode, rating, comments_count, moderation_state, moderation_labels, created_at, updated_at, spoiler, spoiler_tag, is_nsfw, profiles(username, display_name)"
          .replace("profiles(username, display_name)", "image_url, image_urls, image_captions, pinned_until, video_provider, video_upload_id, video_upload_token, video_asset_id, video_playback_id, video_status, video_thumbnail_url, video_duration_seconds, external_video_provider, external_video_id, external_video_url, external_video_title, external_video_thumbnail_url, profiles(username, display_name, selected_title_key, selected_name_color, developer_game_ids)")
      )
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      setIsLoading(true);
      const nextPosts = await loadPostsWithReactions(buildQuery(0), session?.user?.id);
  ...
  }, [followedGameIds, session?.user?.id, buildQuery]);

  const loadMore = useCallback(async () => {
    if (isLoadingMore || !hasMore || !followedGameIds.length) return;

    try {
      setIsLoadingMore(true);
      const nextPosts = await loadPostsWithReactions(buildQuery(offsetRef.current), session?.user?.id);
      offsetRef.current += nextPosts.length;
      setPosts((current) => [...current, ...nextPosts]);
      setHasMore(nextPosts.length === FEED_PAGE_SIZE);
    } catch {
      // Silent — existing posts stay visible
    } finally {
      setIsLoadingMore(false);
    }
  }, [isLoadingMore, hasMore, followedGameIds.length, buildQuery, session?.user?.id]);
  ...
  useRealtimeReload({
    enabled: followedGameIds.length > 0,
    tables: ["posts", "post_reactions", "post_comments", "follows"],
    reload: loadPosts,
  });
...
export function useFriendsFeedPosts(friendUserIds, { limit = 30 } = {}) {
  const { session } = useAuth();
  const [posts, setPosts] = useState([]);
  const [isLoading, setIsLoading] = useState(true);
  const [isLoadingMore, setIsLoadingMore] = useState(false);
  ...
  const uniqueFriendUserIds = useMemo(
    () => [...new Set((friendUserIds ?? []).filter(Boolean))],
    [friendUserIds],
  );
  ...
  const buildQuery = useCallback((offset) =>
    supabase
      ...
      .in("user_id", uniqueFriendUserIds)
      .neq("moderation_state", "hidden")
      .order("created_at", { ascending: false })
      .range(offset, offset + limit - 1),
  [limit, uniqueFriendUserIds]);

  const loadPosts = useCallback(async () => {
    if (!uniqueFriendUserIds.length) {
      setPosts([]);
      setError(null);
      setIsLoading(false);
      setHasMore(false);
  ...
  }, [buildQuery, limit, session?.user?.id, uniqueFriendUserIds.length]);

  const loadMore = useCallback(async () => {
    if (isLoadingMore || !hasMore || !uniqueFriendUserIds.length) return;
    ...
    try {
      ...
      // Keep the current feed visible and allow retry.
    } finally {
      setIsLoadingMore(false);
    }
  }, [buildQuery, hasMore, isLoadingMore, limit, session?.user?.id, uniqueFriendUserIds.length]);
  ...
  useRealtimeReload({
    enabled: uniqueFriendUserIds.length > 0,
    tables: ["posts", "post_reactions", "post_comments", "user_friendships"],
    reload: loadPosts,
  });
...
export function useGamePosts(gameId) {
  ...
  const buildQuery = useCallback((offset) =>
    supabase
      ...
      .neq("moderation_state", "hidden")
      .order("created_at", { ascending: false })
      .range(offset, offset + GAME_POSTS_PAGE_SIZE - 1),
  [numericGameId]);
  ...
  const loadPosts = useCallback(async () => {
    ...
    try {
      ...
      offsetRef.current = nextPosts.length;
      setPosts(nextPosts);
      setHasMore(nextPosts.length === GAME_POSTS_PAGE_SIZE);
      setError(null);
    } catch (nextError) {
      setPosts([]);
      setError(nextError);
  ...
  }, [numericGameId, session?.user?.id, buildQuery]);
  ...
  }, [isLoadingMore, hasMore, buildQuery, session?.user?.id]);

  const updatePostReaction = useCallback((postId, reactionType, previousReaction) => {
    setPosts((current) =>
      current.map((post) => {
        if (post.id !== postId) return post;
        const counts = { ...(post.reactionCounts ?? {}) };
        if (previousReaction && (counts[previousReaction] ?? 0) > 0) {
        ...
        return {
          ...post,
          viewerReaction: reactionType === previousReaction ? null : reactionType,
  ...
  useRealtimeReload({
    enabled: Boolean(numericGameId && !Number.isNaN(numericGameId)),
...
export function usePostSearch(query, { limit = 8 } = {}) {
  const { session } = useAuth();
    ...
    try {
      ...
      const nextPosts = await loadPostsWithReactions(
        supabase
          ...
          .or(`title.ilike.%${normalizedQuery}%,body.ilike.%${normalizedQuery}%,game_title.ilike.%${normalizedQuery}%`)
          .neq("moderation_state", "hidden")
          .order("created_at", { ascending: false })
          .limit(limit),
        session?.user?.id,
  ...
  }, [limit, normalizedQuery, session?.user?.id]);
...
export async function createPost({
  ...
  try {
    ...
    const normalizedImageAssets = Array.isArray(imageAssets) && imageAssets.length > 0
      ? imageAssets
      : imageAsset
        ? [imageAsset]
        : [];

    if (normalizedImageAssets.length > 0) {
      const uploads = [];

      for (let index = 0; index < normalizedImageAssets.length; index += 1) {
        onProgress?.({
          stage: "images",
          index: index + 1,
          total: normalizedImageAssets.length,
          label: `Uploading image ${index + 1} of ${normalizedImageAssets.length}`,
        });

        uploads.push(await uploadPostImage({
          userId,
          asset: normalizedImageAssets[index],
        }));
      }
      ...
      imageUrl = imageUrls[0] ?? null;
      imageMetadata = uploads[0]?.metadata ?? null;
      imageMetadataList = uploads.map((upload) => upload.metadata);
    }
    ...
    const normalizedExternalVideo = externalVideo && externalVideo.provider === "youtube"
      ? {
          provider: "youtube",
          videoId: String(externalVideo.videoId ?? "").trim(),
          url: String(externalVideo.url ?? "").trim() || null,
          title: String(externalVideo.title ?? "").trim() || null,
          thumbnailUrl: String(externalVideo.thumbnailUrl ?? "").trim() || null,
        }
      : null;
    ...
    const data = await invokeEdgeFunction("trusted-post", {
      ...
      videoUploadId,
      videoUploadToken,
      externalVideoProvider: normalizedExternalVideo?.provider ?? null,
      externalVideoId: normalizedExternalVideo?.videoId ?? null,
      externalVideoUrl: normalizedExternalVideo?.url ?? null,
      externalVideoTitle: normalizedExternalVideo?.title ?? null,
      externalVideoThumbnailUrl: normalizedExternalVideo?.thumbnailUrl ?? null,
    });
    ...
    await Promise.all(uploadedImagePaths.map((path) => removePostImage(path)));
...
export async function deletePost({ postId }) {
  await invokeEdgeFunction("trusted-post", {
    action: "delete",
    postId,
  });
}
...
export function usePostComments(postId, isEnabled = true) {
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: commentsError } = await supabase
        .from("post_comments")
        .select(
          "id, post_id, user_id, body, image_url, link_url, link_label, moderation_state, moderation_labels, created_at, updated_at, profiles(username, display_name)"
  ...
  useRealtimeReload({
    ...
    tables: ["post_comments", "comment_reactions"],
    reload: loadComments,
  });
...
export function useEditablePost(postId, isEnabled = true) {
  const { session } = useAuth();
...
export async function createPostComment({ userId, postId, body, imageAsset = null, linkUrl = null, linkLabel = null }) {
  let uploadedImagePath = null;
  try {
    let imageUrl = null;
    if (imageAsset) {
      const { uploadPostImage } = await import("./postMedia");
      const upload = await uploadPostImage({ userId, asset: imageAsset });
    ...
    if (uploadedImagePath) {
      const { removePostImage } = await import("./postMedia");
      await removePostImage(uploadedImagePath).catch(() => {});
    }
    return {
      data: null,
      error: {
...
export async function updatePostComment({ commentId, body, linkUrl = null, linkLabel = null }) {
  try {
    const data = await invokeEdgeFunction("trusted-comment", {
      action: "update",
      commentId,
