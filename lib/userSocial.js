...
export function useMyReviewCount(userId) {
  ...
  const loadReviews = useCallback(async () => {
    ...
    try {
      ...
      const ratedRows = rows.filter((row) => typeof row.rating === "number" && row.rating > 0);
      if (ratedRows.length > 0) {
        const avg = ratedRows.reduce((sum, row) => sum + row.rating, 0) / ratedRows.length;
...
export function useMyReviewsByGame(userId) {
    ...
    try {
      ...
      for (const row of data ?? []) {
        if (row.igdb_game_id && row.rating != null) {
          map.set(row.igdb_game_id, row.rating);
        }
      }
  ...
  useRealtimeReload({ enabled: Boolean(userId), tables: ["posts"], reload: load });
