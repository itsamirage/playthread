  ...
  const loadProfile = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: nextError } = await supabase
        .from("profiles")
        .select(
          "id, username, display_name, avatar_url, bio, created_at, selected_name_color, selected_title_key, developer_game_ids"
        )
        .eq("id", userId)
        .maybeSingle();
...
const ACTIVITY_SELECT =
  "id, user_id, igdb_game_id, game_title, game_cover_url, type, title, body, reaction_mode, rating, comments_count, moderation_state, moderation_labels, created_at, updated_at, spoiler, spoiler_tag, is_nsfw, image_url, image_urls, image_captions, pinned_until, video_provider, video_upload_id, video_upload_token, video_asset_id, video_playback_id, video_status, video_thumbnail_url, video_duration_seconds, external_video_provider, external_video_id, external_video_url, external_video_title, external_video_thumbnail_url, profiles(username, display_name, selected_title_key, selected_name_color, developer_game_ids)";
  ...
  const loadComments = useCallback(async () => {
    ...
    try {
      ...
      const { data, error: nextError } = await supabase
        .from("post_comments")
        .select(
          "id, post_id, user_id, body, image_url, link_url, link_label, moderation_state, moderation_labels, created_at, updated_at, posts(id, title, igdb_game_id, game_title), profiles(username, display_name, selected_title_key, selected_name_color)",
        )
        .eq("user_id", userId)
        .neq("moderation_state", "hidden")
        .order("created_at", { ascending: false })
