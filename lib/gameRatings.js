...
function normalizeStoredRating(value) {
  ...
  // Multiply by 2 to get back to 1-10 scale, then round to nearest 0.5 increment.
  // Using toFixed(1) was producing 9.6 for values like 4.8 (stored float imprecision).
  const raw = Number(value) * 2;
  return Math.round(raw * 2) / 2;
}

function toStoredRating(value) {
...
export async function saveGameRating({ gameId, userId, rating }) {
  ...
  const { error } = await supabase.from("game_ratings").upsert(
    {
      user_id: cleanUserId,
      igdb_game_id: numericGameId,
      rating: toStoredRating(normalizedRating),
    },
