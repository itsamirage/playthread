import { evaluateModerationText } from "../supabase/functions/_shared/trusted.ts";
import { evaluateAvatarSubmission, validateProfileIdentityInput } from "./profileModerationLogic.js";
import { isGeneratedPlaceholderUsername } from "./profileHelpers.mjs";
import { isValidUsername } from "./usernameValidation.js";
import { getContentRemovalUpdate, getContentVisibilityUpdate, normalizeRetentionArgs } from "./adminModerationLogic.js";
...
export function buildContentRemovalWritePlan(flagRow) {
  if (!flagRow?.content_id) {
    throw new Error("That flagged item no longer has a target record.");
  }

  const removal = getContentRemovalUpdate(flagRow.content_type);

  return {
    targetTable: removal.targetTable,
    flagUpdate: {
      status: removal.nextFlagStatus,
    },
    actionInsert: {
      action_type: removal.actionType,
      ...
      metadata_json: {
        ...
        category: flagRow.category,
        origin: flagRow.origin,
        nextFlagStatus: removal.nextFlagStatus,
      },
    },
    result: {
      deleted: true,
      flagStatus: removal.nextFlagStatus,
    },
  };
}
