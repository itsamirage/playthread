...
export async function pickPostImage() {
  ...
  return optimizePostImageAsset(result.assets[0]);
}
...
async function optimizePostImageAsset(asset) {
  ...
  const needsFormatConversion = mimeType === "image/heic" || mimeType === "image/heif";

  if (mimeType === "image/gif" || (!needsFormatConversion && !needsResize && !needsCompression)) {
    return asset;
  }
