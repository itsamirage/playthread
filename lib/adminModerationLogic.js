...
export function getContentRemovalUpdate(contentType) {
  const nextContentType = String(contentType ?? "").trim();

  if (!["post", "comment"].includes(nextContentType)) {
    throw new Error("Only post and comment flags support content removal.");
  }

  return {
    contentType: nextContentType,
    targetTable: nextContentType === "post" ? "posts" : "post_comments",
    nextFlagStatus: "actioned",
    actionType: "delete_content",
    reason: `Flagged ${nextContentType} was removed by moderation.`,
  };
}
