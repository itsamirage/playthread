...
const AGE_RATING_QUERY_TERMS = new Set(["e", "e10", "t", "m", "ao", "esrb"]);
...
export function stripAgeRatingQueryTerms(query) {
  ...
  const contentTerms = queryTerms.filter((queryTerm) => !AGE_RATING_QUERY_TERMS.has(queryTerm));
  return contentTerms.length > 0 ? contentTerms.join(" ") : normalizedQuery;
}
...
export function matchesQueryFuzzily(game, query) {
  const normalizedQuery = stripAgeRatingQueryTerms(query);
