...
export function validatePostImageAsset(asset = {}) {
  const inferredExtension = inferImageExtension(asset);
  const mimeType = String(asset.mimeType ?? "").trim().toLowerCase() || (
    inferredExtension === "jpg" || inferredExtension === "jpeg"
      ? "image/jpeg"
  ...
  if (!mimeType || (!ALLOWED_IMAGE_MIME_TYPES.has(mimeType) && !CONVERTIBLE_IMAGE_MIME_TYPES.has(mimeType))) {
