...
export async function fetchDiscoverGames({ limit = 60, offset = 0 } = {}) {
  const data = await invokeIgdbProxy("discover", { limit, offset });
  return data.games ?? [];
}
...
export async function searchGames(query, { limit = 20, offset = 0 } = {}) {
  const data = await invokeIgdbProxy("search", { query, limit, offset });
  return data.games ?? [];
}
