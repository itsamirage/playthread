...
function normalizeProfile(row) {
  return {
    ...
    developerGameIds: row.developer_game_ids ?? [],
...
function normalizeFlag(row) {
  ...
  const imageUrls = Array.isArray(evidence.image_urls)
    ? evidence.image_urls.map((value) => String(value ?? "").trim()).filter(Boolean)
    : evidence.image_url
      ? [evidence.image_url]
      : [];
  const mediaKind =
    ...
    (evidence.post_type === "clip"
      ...
      : evidence.image_url || ["image", "screenshot"].includes(evidence.post_type)
        ...
        : "text");
  const imageMetadata =
    evidence.image_metadata && typeof evidence.image_metadata === "object"
      ? evidence.image_metadata
      : null;
  ...
  return {
    ...
    userId: row.user_id,
    reporterUserId: row.flagged_by ?? evidence.reporter_user_id ?? null,
    author: row.profiles?.display_name ?? row.profiles?.username ?? "player",
    authorNameColor: row.profiles?.selected_name_color ?? "default",
    reporter: row.reporter_profiles?.display_name ?? row.reporter_profiles?.username ?? null,
    reporterNameColor: row.reporter_profiles?.selected_name_color ?? "default",
    ...
    mediaUrl: imageUrls[0] ?? evidence.video_thumbnail_url ?? null,
    mediaUrls: imageUrls,
    imageMetadata,
  };
}
...
function normalizeGameYouTubeSource(row) {
  return {
    id: row.id,
    gameId: row.igdb_game_id,
    gameTitle: row.game_title,
    gameCoverUrl: row.game_cover_url ?? null,
    channelUrl: row.channel_url,
    channelId: row.channel_id,
    uploadsPlaylistId: row.uploads_playlist_id ?? null,
    channelTitle: row.channel_title ?? null,
    enabled: Boolean(row.enabled),
    autopostStartedAt: row.autopost_started_at,
    lastCheckedAt: row.last_checked_at ?? null,
    lastSeenVideoPublishedAt: row.last_seen_video_published_at ?? null,
...
export function useModerationFlags(currentProfile) {
  ...
  const loadFlags = useCallback(async () => {
    try {
      setIsLoading(true);
      const result = await invokeEdgeFunction("trusted-admin", {
        action: "get_moderation_flags",
      });

      setFlags((result?.flags ?? []).map(normalizeFlag));
...
export function useGameYouTubeSources(currentProfile) {
  ...
  const loadSources = useCallback(async () => {
    try {
      ...
      setSources((data?.sources ?? []).map(normalizeGameYouTubeSource));
      setError(null);
    } catch (nextError) {
      setSources([]);
      setError(nextError);
  ...
  useEffect(() => {
    ...
    loadSources();
  }, [currentProfile?.accountRole, loadSources]);
...
export async function updateDeveloperGames({
  targetUserId,
  developerGameIds,
}) {
  const data = await invokeEdgeFunction("trusted-admin", {
...
export async function updatePostMetadata({
  ...
  return invokeEdgeFunction("trusted-admin", {
...
export async function upsertGameYouTubeSource({
  gameId,
  gameTitle,
  gameCoverUrl = null,
  channelUrl,
  ...
  return data?.source ? normalizeGameYouTubeSource(data.source) : null;
}

export async function disableGameYouTubeSource({ sourceId }) {
  const data = await invokeEdgeFunction("trusted-admin", {
...
export async function ensureYouTubeBot() {
  const data = await invokeEdgeFunction("trusted-admin", {
...
export async function runYouTubeImportNow() {
  return invokeEdgeFunction("youtube-import", {});
}
...
export async function deleteFlaggedContent({ flagId }) {
  const data = await invokeEdgeFunction("trusted-admin", {
