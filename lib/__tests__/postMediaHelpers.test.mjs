...
test("validatePostImageAsset accepts iPhone camera HEIC uploads before conversion", () => {
  assert.doesNotThrow(() =>
    validatePostImageAsset({
      fileName: "IMG_0242.HEIC",
      mimeType: "image/heic",
      fileSize: 2_400_000,
      width: 3024,
      height: 4032,
...
test("validatePostImageAsset infers HEIC uploads when iOS omits mime type", () => {
  assert.doesNotThrow(() =>
    validatePostImageAsset({
      fileName: "IMG_0243.heic",
      fileSize: 2_400_000,
      width: 3024,
      height: 4032,
    }),
...
test("validatePostImageAsset rejects unsupported mime types", () => {
  assert.throws(
    ...
    /Choose a JPG, PNG, WebP, GIF, HEIC, or HEIF image/,
  );
});
