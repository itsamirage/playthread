...
import { processContentRemoval, processContentVisibilityUpdate, processRetentionPrune } from "../trustedAdminService.mjs";
import {
...
test("processContentRemoval deletes content, actions the flag, and records an audit row", async () => {
  const adminClient = createMockAdminClient();
  ...
  const commentDelete = adminClient.operations.find((operation) => operation.table === "post_comments");
  const flagUpdate = adminClient.operations.find(
    (operation) => operation.table === "moderation_flags" && operation.action === "update"
  );
  const actionInsert = adminClient.operations.find(
  ...
  assert.equal(commentDelete.action, "delete");
  assert.deepEqual(commentDelete.filters, [{ column: "id", value: "comment-1" }]);
  assert.equal(flagUpdate.payload.status, "actioned");
  assert.equal(actionInsert.payload.action_type, "delete_content");
  assert.deepEqual(result, { deleted: true, flagStatus: "actioned" });
});
