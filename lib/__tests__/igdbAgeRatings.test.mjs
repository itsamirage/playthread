...
test("isMatureAgeRating only marks ESRB AO labels", () => {
  ...
  assert.equal(isMatureAgeRating([{ rating_category: { organization: 2, rating: "eighteen" } }]), false);
  assert.equal(isMatureAgeRating([{ category: 4, rating: 5 }]), false);
  assert.equal(isMatureAgeRating([], [{ name: "Fantasy" }]), false);
  assert.equal(isMatureAgeRating([], [{ name: "Adult" }]), false);
});
