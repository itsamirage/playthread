...
test("noise control only applies cooldowns to friend and followed-game activity", () => {
  assert.equal(
    getNotificationPushCooldownMinutes(
      {
...
test("friend request notifications aggregate recent requester names", () => {
  const update = buildAggregatedNotificationUpdate(
    {
      entity_id: "older-requester",
      metadata_json: {
        aggregatedCount: 1,
        recentRequesterNames: ["player1"],
      },
    },
    {
      kind: "friend_request",
      entityId: "latest-requester",
      title: "New friend request",
      metadata: {
        requesterName: "player2",
      },
    },
  );

  assert.equal(update.title, "You have new friend requests");
  assert.equal(update.entityType, "profile");
  assert.equal(update.entityId, "latest-requester");
  assert.deepEqual(update.metadata.recentRequesterNames, ["player2", "player1"]);
});
