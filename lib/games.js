...
async function searchGamesWithFallback(query, { limit = 20, offset = 0 } = {}) {
  const searchQuery = stripAgeRatingQueryTerms(query);
  const primaryGames = await searchGames(searchQuery, { limit, offset });
  const fallbackQueries = buildSearchFallbackQueries(searchQuery).slice(0, 4);
  const mergedGames = [];
  ...
  for (const game of ranked) {
    ...
    const releaseYear = String(game.releaseYear ?? "unknown");
    const titleGroupKey = `${normalizedTitle}::${releaseYear}`;
    const existing = titleGroups.get(titleGroupKey);

    if (!existing) {
      titleGroups.set(titleGroupKey, game);
...
function filterGames(games, query, selectedGenre) {
  ...
  const filteredGames = games.filter((game) => {
    ...
    const matchesQuery =
      ...
      normalizeSearchValue(game.title).includes(cleanQuery) ||
      normalizeSearchValue(game.studio).includes(cleanQuery) ||
...
export function useBrowseGames({ query, selectedGenre, hideMatureGames = false }) {
  ...
  const [games, setGames] = useState(sortGamesForBrowse(mockGames));
  const [isLoading, setIsLoading] = useState(!cleanQuery && isIgdbConfigured());
...
export function useGameDetail(gameId, { hideMatureGames = false } = {}) {
  ...
  return {
    game: hideMatureGames && isAdultOnlyGame(game) ? null : game,
    isLoading,
    error,
    source,
  };
