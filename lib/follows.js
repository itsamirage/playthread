...
export const FOLLOW_STATUS_OPTIONS = [
  { key: "have_not_played", label: "Backlog" },
  { key: "currently_playing", label: "Currently Playing" },
  { key: "taking_a_break", label: "Taking a Break" },
  { key: "completed", label: "Completed" },
];

export const ACTIVE_FOLLOW_STATUS_OPTIONS = FOLLOW_STATUS_OPTIONS.filter(
  (option) => option.key !== "have_not_played",
);
      ...
      try {
        ...
        await Promise.all(
          [...coverMap.entries()].map(([gameId, coverUrl]) =>
            invokeEdgeFunction("trusted-user", {
    ...
    if (existingGame) {
      try {
        await invokeEdgeFunction("trusted-user", {
          ...
          gameId,
          gameTitle: existingGame.title ?? game?.title ?? `Game ${gameId}`,
          gameCoverUrl: existingGame.coverUrl ?? game?.coverUrl ?? null,
